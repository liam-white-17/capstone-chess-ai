...
def test_pawn_moves():
    ...
    white_move_1_square = Move(src=(1,0),dest=(2,0),board=board)
    white_move_2_squares = Move(src=(1,0),dest=(3,0),board=board)
    ...
    assert expected == actual
    black_move_1_square = Move(src=(6, 0), dest=(5, 0), board=board)
...
def test_knight_moves():
    ...
    assert Move(src=(0,1),dest=(2,0),board=board) in moves
    assert Move(src=(0,1),dest=(2,2),board=board) in moves
...
def test_queen_moves():
    ...
    assert Move(src=(3,2),dest=(4,3),board=board) in moves
    assert Move(src=(3,2),dest=(2,3),board=board) in moves
    assert Move(src=(3,2),dest=(3,7),board=board) in moves
    assert Move(src=(3,2),dest=(0,2),board=board) in moves
    assert Move(src=(3,2),dest=(0,0),board=board) not in moves
...
def test_king_moves():
    ...
    assert len(moves) == 5
    assert Move(src=(0,1),dest=(0,0),board=board) in moves
    assert Move(src=(0,1),dest=(1,2),board=board) in moves
    assert Move(src=(0,1),dest=(1,1),board=board) in moves
